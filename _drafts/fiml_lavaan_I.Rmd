---
title: 'FIML for Missing Data in Lavaan Part 1: Descriptive Statistics and Correlations'
author: "William Murrah"
date: "June 22, 2017"
output: html_document
---

I attended a workshop recently on how to handle missing data given by Craig Enders. Craig is a bit of a guru on the subject, and has published a great book explaining how to use missing data techniques entitled Applied Missing Data Analysis. The book is both thorough and accessible. He also has an accompanying website that contains code and data for the examples in the book. This code is primarily in Mplus, which can be used to handle missing data either with full information maximum likelihood (FIML) or multiple imputation, the two cutting-edge techniques recommended by many experts these days.

While I also use Mplus, I prefer to use R as much as possible. If you want to do multiple imputation in R, you have several options. But until recently, if you wanted to use FIML in R to run basic models (e.g. regression, ANOVA) there was no simple way to do it. Things are different now, thanks to a wonderful package called lavaan. While there are also other structural equation modeling programs in R that can do FIML (e.g. sem and OpenMx) I find lavaan the easiest to learn and best when you don't really need to use latent variables, but need some way to deal with missing data.

The purpose of this and future posts will be to take some of the examples related to FIML from Craig's website, and translate them into `lavaan'. I hope this will give those who don't have access to Mplus (which is very expensive!) the ability to work through the examples using free and open source software.
...
You will also want to read over the lavaan documentation and visit the very helpful lavaan website to take advantage of the tutorials there. With these resources at your disposal, you should be able to replicate the examples in lavaan. Here, I walk through the major sections of the R code. This is the same code found in the github repository in the R file entitled FIMLdescriptivesCorrelations.R.
...
To fit the model, I use the lavaan fit function. This function takes the first two arguments model and data. The third argument is missing ='fiml', which tells lavaan to use FIML (the default is to use listwise deletion).
...
FIML FOR MISSING DATA IN LAVAAN PART 2: REGRESSION ANALYSIS